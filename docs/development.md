...
Samostatný Vite náhled podporuje `?socialDemo=1` pro frontu reálných párových rozhovorů, `?groupDemo=sofa`, `?groupDemo=meeting` a `?groupDemo=standing` pro deterministické idle skupiny, `?crowdDemo=1` pro deterministický head-on test dvou agentů, `?workDemo=1` pro detail pracovního posedu, `?kitchenDemo=1` pro cestu ke kávovaru a animaci pití, `?couchDemo=1` pro přechod z walkable bodu na vizuální kotvu sedáku, `?ownerDemo=1` pro WASD chůzi majitele a následný návrat autonomie, `?attentionDemo=1` pro periodické upozornění agenta ve stavu `waitingForUser` a `?retirementDemo=1` pro zrychlený lifecycle odcházejícího podagenta. Crowd scénář musí skončit bočními waypointy a bezpečným minutím, nikoli posouváním stojící postavy.
